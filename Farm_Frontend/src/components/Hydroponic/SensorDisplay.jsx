...
function SensorDisplay() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:4000/api/temperature/data');
        const { temperature, humidity, soil_moisture } = response.data;
        setData([...data, { temperature, humidity, soil_moisture }]); // Add new data point
